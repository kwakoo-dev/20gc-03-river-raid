...
func draw_level_start_v2(y : int, corridor_end_y : int) -> int:
	if y < corridor_end_y:
		_put_grass(_get_river_banks_line(y))
		return 1
	elif y >= corridor_end_y && not (bank_left == OPENING_LEFT && bank_right == OPENING_RIGHT):
		_adjust_river_banks(OPENING_LEFT, OPENING_RIGHT, y)
		_put_grass(_get_river_banks_line(y))
		return 1
	else:
...
func draw_river_v2(y : int, river_end_y : int) -> int:
	if y <= river_end_y:
		return 0
	_put_grass(_get_river_line(y))
	return 1
...
func draw_island_v2_start(y : int) -> void:
	island_left = DEFAULT_BANK_LEFT
	island_right = DEFAULT_BANK_RIGHT

func draw_island_v2(y : int) -> int:
	_put_grass(_get_island_line(y))
	return 1
